...
import com.nelson.weather.Constants;
...
import com.nelson.weather.sharelibrary.request.BitmapAsyncTask;
...
public class ShareByWeixin extends ShareBase {
    ...
    public ShareByWeixin(Context context, int channel) {
        ...
        api = WXAPIFactory.createWXAPI(context.getApplicationContext(), Constants.APP_ID);
    ...
    private void start() {
        if (api.isWXAppInstalled()) {
            ...
            if (!TextUtils.isEmpty(imgUrl)) {
                ...
                } else if("default".equals(imgUrl)){
                    shareImg(TransportBitmap.getInstance().getBitmap(),listener);
                }else {
                    //本地图片
                    if (data.isShareBigImg()) {
    ...
    private void send(Bitmap bitmap) {
        ...
        req.userOpenId =  ManifestUtil.getWeixinKey(this.context);
    }
