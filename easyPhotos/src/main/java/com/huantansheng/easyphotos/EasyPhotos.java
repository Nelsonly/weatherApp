...
public class EasyPhotos {
    ...
    /**
     * 开启带底部预览的图片选择（多选），调用者需实现PuzzleSelectorActivity2.OnSelectPhotosForPuzzleDoneListener
     * @param activity listener
     * @param imageEngine 图片加载引擎
     */
    public static void startSelectorForPuzzle(Activity activity, ImageEngine imageEngine){
        if (Setting.imageEngine != imageEngine) {
            Setting.imageEngine = imageEngine;
        }
        PuzzleSelectorActivitySecond.start(activity);
    }
